...
# --------------------------------------------------------------
# CONFIG MANAGEMENT (Login Mode Toggle)
# --------------------------------------------------------------
    ...
    try:
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, 'r') as f:
                return json.load(f)
        return {"free_login_enabled": False}
    except:
        return {"free_login_enabled": False}
...
def login_page():
    config = get_config()
    if config.get('free_login_enabled'):
...
def do_login():
    # Check if free login request
    if request.form.get('free_login') == 'true':
        config = get_config()
        if config.get('free_login_enabled'):
            session['logged_in'] = True
            return redirect(url_for('index_page'))
        else:
            return jsonify({"status": "error", "message": "Free login disabled"}), 401
...
@app.route('/admin/toggle_login_mode', methods=['POST'])
def toggle_login_mode():
    try:
        enabled = request.json.get('enabled', False)
        if set_config_value('free_login_enabled', enabled):
            return jsonify({"ok": True})
        else:
            return jsonify({"ok": False, "message": "Config save failed"})
    except Exception as e:
        return jsonify({"ok": False, "message": str(e)})
